...
interface Getters {
  ...
  INFO: UseMemoizeReturn<
    Promise<ChapterInfo>,
    [id: number, locale: LocaleCode]
  >;
}
...
export default defineComponent({
  ...
  async setup(props, { expose, emit }) {
    ...
    const data = computed<{ page: number; verses: Verses[] }[]>(() => {
      const page: any = collect(paginate.value.data).groupBy(
    ...
    const pages = computed<number[]>(() => {
      const page = collect(collection.value.flat()).groupBy(
        (item) => item.page_number
      ).items;
      ...
      return new Promise((resolve) => {
        ...
        setTimeout(() => {
          requestAnimationFrame(() => {
            paginate.value.data.push(
              ...collection.value[paginate.value.currentPage]
            );
    ...
    function scroll(offset: number): Promise<void> {
      return new Promise((resolve) => {
        offset =
          offset -
          ((document.querySelector("nav.navbar") as HTMLElement)
            ?.offsetHeight || 0) +
          80;
    ...
    function scrollToVerse(verseNumber: number): Promise<void> {
      return new Promise((resolve, reject) => {
        (async () => {
          ...
          const verseKey = [props.chapter.id, verseNumber]
            .map(String)
            .join(":");

          while (true) {
            const versesWrapper: HTMLElement[] = Array.from(
              root.value?.querySelectorAll("[data-verse-keys]") || []
            );
            const versesWrapperEl = versesWrapper.find((el) =>
              (el.dataset.verseKeys?.split(",") || []).includes(verseKey)
            );
          ...
          nextTick(() => {
            const currentRef = versesRef.value.find((ref) =>
              ref.getKey().split(",").includes(verseKey)
            );

            currentRef?.setVisible(true);

            nextTick(() => {
              const verseEl = root.value?.querySelector(
    ...
    function scrollToPage(pageNumber: number) {
      return new Promise((resolve, reject) => {
        (async () => {
          ...
          while (
            !data.value.find((row) => row.page == pageNumber) &&
            (await loadMore(0))
          ) {
            //
          }
          ...
          nextTick(() => {
            const pageEl = root.value?.querySelector(
  ...
  render() {
    return (
      <>
        <Banner chapter={this.chapter} chapterInfo={this.chapterInfo!} />
