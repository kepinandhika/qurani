...
export default defineComponent({
  setup() {
    ...
    // Tambahkan state untuk minimize card
    const isGroupsMinimized = ref(true);
    const isFriendsMinimized = ref(true);

    // Fungsi untuk toggle minimize pada Card Grup
    function toggleGroupsCard() {
      isGroupsMinimized.value = !isGroupsMinimized.value;
    }
    // Fungsi untuk toggle minimize pada Card Teman
    function toggleFriendsCard() {
      isFriendsMinimized.value = !isFriendsMinimized.value;
    }
    ...
    function addFriend() {
      ...
      // Jika nama yang dicari adalah "Liko" atau "Galang", maka pengguna dianggap tidak ada.
  ...
  render() {
    return (
      <MainLayout>
        {/* Card untuk Grup */}
        <Card class="mb-4 bg-primary bg-primary text-white" headerClasses="text-center">
          {{
            header: () => (
              <div class="d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-light text-primary" onClick={this.toggleGroupsCard}>
                  {this.isGroupsMinimized ? this.t("general.kecil") : this.t("general.besar")}
                </button>
              </div>
            ),
            default: () =>
              !this.isGroupsMinimized && (
                <div class="p-3">
                  {/* Form tambah grup */}
                  ...
                  {this.groups.map((group) => (
                    <div key={group.id} class="mb-3 border-bottom pb-2">
                      <div
                        ...
                        <div class="mt-2 ms-3">
                          ...
                          <ul class="list-unstyled mb-0">
                            {group.members.map((member, idx) => (
                              <li key={idx} class="d-flex justify-content-between align-items-center">
                                <span>{member}</span>
                                <div class="d-flex gap-2">
                                  <button
                                    ...
                                    onClick={() =>
                                      this.removeMemberFromGroup(group.id, member)
                                    }
                                  >
          ...
          {{
            header: () => (
              <div class="d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-light text-primary" onClick={this.toggleFriendsCard}>
                {this.isGroupsMinimized ? this.t("general.kecil") : this.t("general.besar")}
                </button>
              </div>
            ),
            default: () =>
              !this.isFriendsMinimized && (
                <div class="p-3">
                  {/* Form tambah teman */}
                  <div class="mb-3 d-flex gap-2">
                    <input
        ...
        {/* Card Favorit Surah */}
        <Card class="mb-4 bg-primary bg-primary text-white" headerClasses="d-flex justify-content-between">
          {{
            header: () => (
